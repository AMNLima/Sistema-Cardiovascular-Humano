...
Lp = 0.0033;
for i=1:length(t)-1
    Pve(i) = E(i)*(Vve(i)-V0); % PressÃ£o no ventrÃ­culo esquerdo
    ...
    [A, B] = changeDiodes(Pao(i), Pae(i), Pve(i), E(i), Pc(i), alpha, Pex(i), Vve(i), Qo(i)); % FunÃ§Ã£o diodo
...
legend('Pao','Pae', 'Pve')
grid on
title('Simulaçãoo do modelo 0D do sistema cardiovascular humano')
ylabel('Pressão (mmHg)')
xlabel('tempo (s)')
